...
void TRSACT_print (TRSACT *T, QUEUE *occ, PERM *p){
  ...
  FLOOP (i, 0, occ? occ->t: T->T.t){
    ...
    if ( occ ) rsafe_printf(QUEUE_INTF "::: ", t);
    for (j=0; j<T->T.v[t].t ; j++){
      e = T->T.v[t].v[j];
      rsafe_printf(QUEUE_INTF, p? p[e]: e);
      if ( T->T.w ) rsafe_printf("(" WEIGHTF ")", T->T.w[t][j]);
      rsafe_printf(",");
    }
    if ( T->w ) rsafe_printf(" :" WEIGHTF " ", T->w[t]);
    rsafe_printf(" (" QUEUE_INTF ")\n", T->T.v[t].end);
  }
}
