...
void SETFAMILY_load_weight (SETFAMILY *M){
  ...
  VEC_ID i;
  QUEUE_ID j;
  if ( M->flag&LOAD_TPOSE ) LCM_ERROR("transope and weight can't be specified simultaneously", EXIT);
...
void SVEC_print (FILE *fp, SVEC *V){
  SVEC_ELE *x;
  MQUE_FLOOP (*V, x) rsafe_fprintf (fp, "("QUEUE_IDF","SVEC_VALF") ", (*x).i, (*x).a);
  fputc ('\n', fp);
}
...
void SETFAMILY_print_WEIGHT (FILE *fp, SETFAMILY *M){
  if ( M->w ){
     rsafe_printf(","); fprint_WEIGHT (stdout, M->w[i][j]); }
  rsafe_printf("\n");
}
*/
