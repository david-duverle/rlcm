...
/*  error routine  */
// 'error' had to be renamed to avoid collision
#define LCM_ERROR(mes,x)        do{ERROR_MES=mes;rsafe_fprintf(stderr,"%s\n",mes);x;}while(0)
#define error_num(mes,n,x)  do{ERROR_MES=mes;rsafe_fprintf(stderr,"%s: %g\n",mes,(double)(n));x;}while(0)
#define error_range(f,x,y,mes)  do{if(!RANGE((x),(f),(y))){ERROR_MES=mes;rsafe_fprintf(stderr,"%s: %g\n",mes,(double)(f));EXIT;}}while(0)
#define error_str(mes,s,x)  do{ERROR_MES=mes;rsafe_fprintf(stderr,"%s: %s\n",mes,s);x;}while(0)
#define print_err(...)      rsafe_fprintf(stderr,__VA_ARGS__)
#define print_mes(S,...)  do{if(((S)->flag)&1)rsafe_fprintf(stderr,__VA_ARGS__);}while(0)
//#define print_fname(s,fname,...)  do{if(fname)rsafe_fprintf(stderr,s,fname,__VA_ARGS__);}while(0)
#define print_fname(s,fname,...)  do{if(fname)rsafe_fprintf(stderr,s,fname);}while(0)
...
#ifdef _cplusplus_
#define   malloc2(f,b,x)     do{if(!((f)=(typeof(f))malloc(((size_t)sizeof((f)[0]))*(b)))){rsafe_fprintf(stderr,"memory allocation error: line %d" #f " (" LONGF " byte)\n",__LINE__,(LONG)((LONG)sizeof((f)[0])*(b)));ERROR_MES="out of memory";x;}}while(0)
#define   calloc2(f,b,x)     do{if(!((f)=(typeof(f))calloc(sizeof((f)[0]),b))){rsafe_fprintf(stderr,"memory allocation error: line %d" #f " (" LONGF " byte)\n",__LINE__,(LONG)((LONG)sizeof((f)[0])*(b)));ERROR_MES="out of memory";x;}}while(0)
#define   realloc2(f,b,x)    do{if(!(f=(typeof(f))realloc(f,((size_t)sizeof((f)[0]))*(b)))){rsafe_fprintf(stderr,"memory allocation error: line %d" #f " (" LONGF " byte)\n",__LINE__,(LONG)((LONG)sizeof((f)[0])*(b)));ERROR_MES="out of memory";x;}}while(0)
#else
#define   malloc2(f,b,x)     do{if(!((f)=malloc(((size_t)sizeof((f)[0]))*(b)))){rsafe_fprintf(stderr,"memory allocation error: line %d" #f " (" LONGF " byte)\n",__LINE__,(LONG)((LONG)sizeof((f)[0])*(b)));ERROR_MES="out of memory";x;}}while(0)
#define   calloc2(f,b,x)     do{if(!((f)=calloc(sizeof((f)[0]),b))){rsafe_fprintf(stderr,"memory allocation error: line %d: " #f " (" LONGF " byte)\n",__LINE__,(LONG)((LONG)sizeof((f)[0])*(b)));ERROR_MES="out of memory";x;}}while(0)
#define   realloc2(f,b,x)    do{if(!(f=realloc(f,((size_t)sizeof((f)[0]))*(b)))){rsafe_fprintf(stderr,"memory allocation error: line %d: " #f " (" LONGF " byte)\n",__LINE__, (LONG)((LONG)sizeof((f)[0])*(b)));ERROR_MES="out of memory";x;}}while(0)
#endif
...
#define   ARY_PRINT(f,x,y,a) do{FLOOP(common_size_t,x,y)rsafe_printf(a,(f)[common_size_t]);rsafe_printf("\n");}while(0)
#define   ARY_FPRINT(fp,f,x,y,a) do{FLOOP(common_size_t,(size_t)x,(size_t)y)rsafe_fprintf((FILE *)fp,a,(f)[common_size_t]);fputc('\n',(FILE *)fp);}while(0)
...
#define   ST_PRINT(S,a,x,y,f) do{FLOOP(common_size_t,x,y)rsafe_printf(f,(S)[common_size_t].a );rsafe_printf("\n");}while(0)
...
#ifdef _MSC_
 #define   fopen2(f,a,b,x)     do{fopen_s(&f,a,b);if(!f){ERROR_MES="file open error";rsafe_fprintf(stderr,"file open error: file name %s, open mode %s\n",a,b);x;}}while(0)
#else
 #define   fopen2(f,a,b,x)     do{if(!((f)=fopen(a,b))){ERROR_MES="file open error";rsafe_fprintf(stderr,"file open error: file name %s, open mode %s\n",a,b);x;}}while(0)
#endif
...
#define   ARY_SCAN(num,type,fp,d)  do{(num)=0;do{do{(type) FILE2_read_##type(&(fp));}while((FILE_err&((d)*5))==5);if(RANGE(5+(int)(d),FILE_err,6))break;(num)++;}while((FILE_err&(3-(int)(d)))==0);}while(0)
#define   ARY_READ_LINE(f,type,num,fp)  do{(num)=0;do{(f)[num]= (type) FILE2_read_##type(&(fp));(num)+=(FILE_err&4)?0:1;}while((FILE_err&3)==0);}while(0)
#define   ARY_READ(f,type,num,fp)  do{FLOOP(common_size_t,0,(size_t)num){do{(f)[common_size_t]=(type) FILE2_read_##type(&(fp));}while((FILE_err&6)==4);if(FILE_err&2)break;}}while(0)
...
#define EXECSUB(func,flag,err,fmt,...) do{\
 rsafe_sprintf(common_comm, fmt, __VA_ARGS__);\
...
#define REMOV(dir,fn) do{ rsafe_sprintf(common_comm,"%s%s",dir,fn); remove (common_comm); }while(0)
#define RENAM(dir,fn,fn2) do{ rsafe_sprintf(common_comm,"%s%s",dir,fn); rsafe_sprintf(common_comm2,"%s%s",dir,fn2); rename (common_comm, common_comm2); }while(0)
