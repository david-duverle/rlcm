...
void ITEMSET_last_output (ITEMSET *I){
    ...
    if ( !(I->flag & SHOW_MESSAGE) )
        return;  // "no message" is specified
    if ( I->topk.end > 0 ){
...
//        rsafe_printf("###\n");
    ...
    if ( I->flag&ITEMSET_ITERS2 )
        print_err (", iters2=" LONGF, I->iters2);
  print_err ("\n");
}
...
//  rsafe_printf("[ratio] %f, p=%f, (%f/ %f), %d(%d) <= ", I->ratio_lb, p, w[item], I->frq, I->perm[item], I->itemflag[item]);
