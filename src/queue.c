...
void QUEUE_rm (QUEUE *Q, QUEUE_ID j){
  if ( Q->s <= Q->t ){
    if ( j < Q->s || j >= Q->t ) LCM_ERROR ("QUEUE_rm: j is out of queue", EXIT);
  } else if ( j < Q->s && j >= Q->t ) LCM_ERROR ("QUEUE_rm: j is out of queue", EXIT);
  QUEUE_DECREMENT(*Q,Q->t);
  Q->v[j] = Q->v[Q->t];
}
...
void QUEUE_perm_print (QUEUE *Q, QUEUE_ID *q){
  QUEUE_ID i;
  for ( i=Q->s ; i!=Q->t ; ){
    rsafe_printf(QUEUE_INTF" ", q[Q->v[i]]);
    QUEUE_INCREMENT(*Q,i);
  }
  rsafe_printf("\n");
}
...
void QUEUE_print_ (QUEUE *Q){
  QUEUE_ID i;
  rsafe_printf("s="QUEUE_IDF",t="QUEUE_INTF": ", Q->s, Q->t);
  for ( i=Q->s ; i!=Q->t ; ){
    rsafe_printf(QUEUE_INTF" ",Q->v[i]);
    QUEUE_INCREMENT(*Q,i);
  }
...
void QUEUE_print__ (QUEUE *Q){
  QUEUE_ID i;
  rsafe_printf("s="QUEUE_IDF",t="QUEUE_IDF": ", Q->s, Q->t);
  for ( i=Q->s ; i!=Q->t ; i++ ) rsafe_printf(QUEUE_INTF" ",Q->v[i]);
  rsafe_printf("\n");
}
