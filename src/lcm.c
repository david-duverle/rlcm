...
/***********************************************************************/
void LCM_read_param (int argc, char *argv[], LCM_PROBLEM *PP){
  ITEMSET *II = &PP->II;
  int c=1, f=0;
  if ( argc < c+3 ){ LCM_error_fn (); return; }
  ...
  else LCM_ERROR ("one of F, C, M has to be given", EXIT);
  ...
  while ( argv[c][0] == '-' ){
      ...
      rsafe_printf("OPT: %c [= %s]\n", argv[c][1], argv[c+1]);

    switch (argv[c][1]){
      case 'K': if ( PP->problem & PROBLEM_MAXIMAL )
          LCM_ERROR ("M command and -K option can not be given simltaneously", EXIT);
    ...
    c += 2;
    if ( argc < c+2 ){ LCM_error_fn (); return; }
  }

    NEXT:;
    if ( (f&3)==3 || (f&5)==5 || (f&6)==6 ) LCM_ERROR ("-f, -F, -a, -A, -p, -P, -r and -R can not specified simultaneously", EXIT);
    if ( f && (II->flag & ITEMSET_PRE_FREQ) ) BITRM (II->flag, ITEMSET_PRE_FREQ);

    if ( ( PP->problem & PROBLEM_CLOSED ) && PP->SG.fname )
      LCM_ERROR ("closed itemset mining does not work with item constraints", EXIT);
    ...
    rsafe_printf("INPUT: %s\n", argv[c]);
    PP->TT.fname = argv[c];
    
    if ( II->topk.end==0 ) {
        rsafe_printf("ARGWEIGHT: %d\n", atof(argv[c+1]));
        II->frq_lb = (WEIGHT)atof(argv[c+1]);
    }
    
    if ( argc>c+2 ) {
        PP->output_fname = argv[c+2];
        rsafe_printf("OUTPUT: %s\n", argv[c+2]);
    }
...
/*********************************************************************/
void LCM_add_item (LCM_PROBLEM *PP, QUEUE *Q, QUEUE_INT item){
  QUEUE_INT *x;
  QUE_INS (*Q, item);
...
/*********************************************************************/
void LCM_del_item (LCM_PROBLEM *PP, QUEUE *Q){
...
void LCM_reduce_occ_by_posi_equisupp (LCM_PROBLEM *PP, QUEUE *occ, QUEUE_INT item, QUEUE_INT full){
  QUEUE_ID ii=0;
  TRSACT *TT = &PP->TT;
  ITEMSET *II = &PP->II;
  QUEUE_INT *x, *y, *z, cnt;
 ...
 */
QUEUE_INT LCM_maximality_check (LCM_PROBLEM *PP, QUEUE *occ, QUEUE_INT item, QUEUE_INT *fmax, QUEUE_INT *cnt){
  ITEMSET *II = &PP->II;
  TRSACT *TT = &PP->TT;
...
/*************************************************************************/
void LCM (LCM_PROBLEM *PP, int item, QUEUE *occ, WEIGHT frq, WEIGHT pfrq){
  ITEMSET *II = &PP->II;
  TRSACT *TT = &PP->TT;
...
//rsafe_printf("itemset: %f ::::", II->frq); QUEUE_perm_print (&II->itemset, II->perm);
//QUEUE_print__ ( occ );
//rsafe_printf("itemset: %f ::::", II->frq); QUEUE_perm_print (&II->itemset, II->perm);
//rsafe_printf("itemset: %f ::::", II->frq); QUEUE_print__ (&II->itemset);
//FLOOP (i, 0, item) rsafe_printf("%1.2f, ", PP->occ_w[i]); rsafe_printf("\n ### ");
//FLOOP (i, item, II->item_max) rsafe_printf("%1.2f, ", PP->occ_w[i]); rsafe_printf("\n");

//rsafe_printf("add:"); QUEUE_perm_print (&II->add, II->perm);
//for (i=0 ; i<II->imax ; i++ ) rsafe_printf("%d(%d) ", II->perm[i], II->itemflag[i]); rsafe_printf("\n");
...
// rsafe_printf("add: "); QUEUE_print__ ( &II->add);
...
void LCM_init (LCM_PROBLEM *PP){
  ...
  PP->clms = ((PP->problem&PROBLEM_FREQSET)&&(II->flag&ITEMSET_RULE)==0);
    ...
    PP->th = (II->flag&ITEMSET_RULE)?
        ((II->flag&ITEMSET_RULE_INFRQ)? -WEIGHTHUGE: II->frq_lb * II->ratio_lb )
        : II->frq_lb;  // threshold for database reduction
    
    if ( TT->flag&TRSACT_SHRINK ) {

        
        QUEUE q = QUEUE_dup_ (&TT->OQ[TT->T.clms]); // preserve occ
        ...
        QUEUE_alloc (&PP->oo, TT->T.t);
        ARY_INIT_PERM(PP->oo.v, TT->T.t);
        PP->oo.t = TT->T.t;
    }
...
//    Rprintf("Type of args: %d (VECSXP: %d)", TYPEOF(args), TYPEOF(args) == VECSXP);
...
//    Rprintf("%d elements\n", argc);
    ...
    for(i = 0; i < argc; i++) {
        argv[i] = CHAR(STRING_ELT(VECTOR_ELT(args, i), 0));
//        Rprintf("read element %d: %s\n", i, argv[i]);
    }
    ...
    LCM_read_param (argc, (char **) argv, &PP);
    ...
    if ( ! ERROR_MES ){
        LCM_init (&PP);
        if ( ! ERROR_MES )
            LCM (&PP, TT->T.clms, &PP.oo, TT->total_w_org, TT->total_pw_org);
        else
            Rprintf("Error: %s\n", ERROR_MES);
//        ITEMSET_last_output (II);
        ...
        ITEMSET_merge_counters (II);
        if (II->flag & SHOW_MESSAGE) {
            if ( II->topk.end > 0 ){
                i = AHEAP_findmin_head (&II->topk);
                val = AHEAP_H(II->topk, i);
            }
            else {
                FLOOP(i, 0, II->itemset.end+1){
                    n += II->sc[i];
                    if ( II->sc[i] != 0 )
                        nn = i;
                }
                ...
                print_err ("iters=" LONGF, II->iters);
                if ( II->flag&ITEMSET_ITERS2 )
                    print_err (", iters2=" LONGF, II->iters2);
                print_err ("\n");
            }
        }
