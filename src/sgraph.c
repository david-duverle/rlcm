...
void SGRAPH_print (SGRAPH *G){
  ...
  rsafe_printf("#node "VEC_IDF" ,#edge %zd ,#arc %zd\n", SGRAPH_NODE_NUM(*G), G->edge.eles, G->in.eles);
  FLOOP (i, 0, SGRAPH_NODE_NUM(*G)){
    rsafe_printf("NODE "VEC_IDF" ", i);
    if ( G->node_w ){ rsafe_putchar ('('); print_WEIGHT (G->node_w[i]); rsafe_putchar (')'); }
    rsafe_printf(" >>\n");
    if ( G->edge.v && G->edge.v[i].t ){
      rsafe_printf("    edge      : ");
      for (j=0; j<G->edge.v[i].t ; j++){
        e = G->edge.v[i].v[j];
        rsafe_printf(VEC_IDF, e);
        if ( G->edge.w ){ rsafe_putchar ('('); print_WEIGHT (G->edge.w[i][j]); rsafe_putchar (')'); }
        rsafe_putchar (',');
      }
      rsafe_putchar ('\n');
    }
    if ( G->in.w ){
      if ( G->in.v[i].t ){
        ...
        for (j=0; j<G->in.v[i].t ; j++){
          e = G->in.v[i].v[j];
          rsafe_printf(VEC_IDF, e);
          if ( G->in.w ){ rsafe_putchar ('('); print_WEIGHT (G->in.w[i][j]); rsafe_putchar (')'); }
          rsafe_putchar (',');
        }
        rsafe_putchar ('\n');
      }
    ...
    if ( G->out.w ){
      if ( G->out.v[i].t ){
        rsafe_printf("    out-arc      : ");
        for (j=0; j<G->out.v[i].t ; j++){
          e = G->out.v[i].v[j];
          rsafe_printf(VEC_IDF, e);
          if ( G->out.w ){ rsafe_putchar ('('); print_WEIGHT (G->out.w[i][j]); rsafe_putchar (')');}
          rsafe_putchar (',');
        }
        rsafe_putchar ('\n');
      }
...
void SGRAPH_sort_node (SGRAPH *G, int flag){
  PERM *tmp;
  tmp = qsort_perm_VECt ((VEC *)(G->edge.v), G->edge.t, (int) (flag == 1 ? sizeof(QUEUE) : -sizeof(QUEUE)));
